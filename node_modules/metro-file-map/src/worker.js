...
async function worker(
  ...
  if (
    ...
    hasteImplModulePath = data.hasteImplModulePath; // $FlowFixMe[unsupported-syntax] - dynamic require
  ...
  let content;
    /*: ?string */
  let dependencies;
    /*: WorkerMetadata['dependencies'] */
  let id;
    /*: WorkerMetadata['id'] */
  let module;
    /*: WorkerMetadata['module'] */
  let sha1;
    /*: WorkerMetadata['sha1'] */
  ...
  if (filePath.endsWith(PACKAGE_JSON)) {
    ...
    if (computeDependencies) {
      dependencies = Array.from(
        data.dependencyExtractor != null // $FlowFixMe[unsupported-syntax] - dynamic require
